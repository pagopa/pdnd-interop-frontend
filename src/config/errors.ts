...
export const getMappedError = (errorCode: string) => {
  const { error } = splitServiceCodeAndMessageCode(errorCode)
  const errorMessages: errorDictType = {
...
const splitServiceCodeAndMessageCode = (errorCode: string): { service: string; error: string } => {
