...
export function useTOSAgreement(jwt: JwtUser | undefined, isSupport: boolean) {
  ...
  const hasAcceptedTOS = true //userTOSConsent?.firstAccept && userTOSConsent?.isUpdated
  const hasAcceptedPP = true //userPPConsent?.firstAccept && userPPConsent?.isUpdated
