...
describe('ProviderEServiceDetailsAlerts', () => {
  ...
  it('hides delegated archiving request alerts but keeps the archived status alert when state is ARCHIVED and the request targets the current descriptor', () => {
    mockUseJwt({ jwt: { organizationId: 'delegate-id' } })
  ...
  it('keeps delegated e-service archiving alerts when state is ARCHIVED but the request does not target the current descriptor', () => {
    mockUseJwt({ jwt: { organizationId: 'delegate-id' } })
