...
type ProviderEServiceTemplateUpdateAttributesDrawerProps = {
  isOpen: boolean
  onClose: () => void
...
export const ProviderEServiceTemplateUpdateAttributesDrawer: React.FC<
  ProviderEServiceTemplateUpdateAttributesDrawerProps
> = ({ isOpen, onClose, attributes, attributeKey }) => {
  ...
  const { mutate: updateAttributes } = TemplateMutations.useUpdateAttributes()
  ...
  const canAttributeBeRemoved = (groupIdx: number, descriptorAttribute: DescriptorAttribute) => {
    return !attributes[attributeKey][groupIdx].some((att) => att.id === descriptorAttribute.id)
  }
